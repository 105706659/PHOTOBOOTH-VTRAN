(function(){
  const state = {
    ...
    shots: [],           // all 8 raw captures from this session
  ...
  document.getElementById('btnCancel').addEventListener('click', () => {
    state.autoCaptureCancelled = true;
    stopCamera();
  ...
  async function runCountdown(){
    for (let i = 5; i >= 1; i--){
      if (state.autoCaptureCancelled) return;
  ...
  async function runAutoCaptureSequence(){
    state.autoCaptureCancelled = false;
  ...
  // === SELECT FAVORITES (pick state.shotCount out of the 8 captures) ===
  ...
  document.getElementById('btnRetakeAll').addEventListener('click', async () => {
    state.shots = [];
    state.pickedIndices = [];
    state.selectedShots = [];
    updateThumbnails();
